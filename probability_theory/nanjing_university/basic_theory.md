...
- 公式
  ...
  > 
  > $P(ABC) = P(A) \frac {P(AB)} {P(A)} \frac {P(ABC)} {P(AB)} = P(A)P(AB|A)P(C|AB)$

- ex
  
  > 对某产品进行三次破坏性实验，产品没有第一种实验的概率为0.3；产品通过第一种实验，但是没有通过第二种实验的概率为0.2；产品通过第一二两种实验，但是没有通过第三种实验的概率为0.1，事件A表示没有通过三种实验，求P(A)
  > 
